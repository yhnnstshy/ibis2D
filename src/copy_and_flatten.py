...
def copy_files_recursive(indir, outdir, exclude):
    ...
    for root, dirnames, filenames in os.walk(indir):
        ...
        for f in filenames:
            ...
            if (cnt > 0):
                ...
                nexclude += 1
                continue
            destname = os.path.join(outdir, f)
            if os.path.isfile(destname):
                logger.info('%s already exists, oh no!', destname)
                continue
            shutil.copy2(fullname, destname) # copies metadata also
            npass += 1

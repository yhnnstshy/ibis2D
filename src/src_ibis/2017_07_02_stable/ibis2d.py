...
def write_boundary_thumbnails(outdir, results_dict, points_dict):
    ...
    tups = [ (float(results_dict[k]['power']), k) for k in results_dict.keys() ]
    ...
    edgecolors = [ 'none' ]
    for (o, t, f, e) in zip(orders, titles, filenames, edgecolors):
        # use golden mean
        n = len(o)
        phi = 0.5 * (1.0 + math.sqrt(5.0))
        nr = math.ceil(math.sqrt(float(n)/phi))
        nc = math.ceil(math.sqrt(float(n)*phi))
        logger.info('golden mean n %d nr %d nc %d tot %d', n, nr, nc, nr*nc)
        colors = mpcm.rainbow(np.linspace(0,1,n))
        ...
        for (k, clr) in zip(o, colors):
            ...
            newy = pts[:,1] + dy
            xy = zip(newx, newy)
            axes.add_patch(Polygon(xy, closed=True, facecolor=clr, edgecolor=e) )
            i = i + 1
        
        axes.autoscale_view()    
        # plt.gca().invert_yaxis()
        plt.axis('off')
        plt.savefig(os.path.join(outdir, f))
        plt.close()
...
def write_thermometers(outdir, results_dict, points_dict):
    ...
    for a in ANNOTS:
        for d in DAYNUMS:
            ...
            plt.xlabel(r'$\Delta$' + ' Protein Expression')
...
def analyze_results(outdir, results_dict):
    results_frame = pd.DataFrame(results_dict)
    # logger.info('results %s', str(results_frame))

...
def normalize_points(points_rfft, area):
    ...
    points_normradius = np.zeros( (n, dim) )
    ...
    facradius = 1.0 / math.sqrt(area / math.pi )
    rfft_normradius = facradius * rfft_centered
    ...
    for k in range(dim):
        points_normradius[:,k] = np.fft.irfft(rfft_normradius[:,k])
        points_normpower[:,k] = np.fft.irfft(rfft_normpower[:,k])

    power_normradius = np.zeros(nk)
    power_normpower = np.zeros(nk)
    # normalize so that a unit circle has power = 1 for first component
    # multiply by 2/n^2
    prefac = 2.0/(n*n)
    for k in range(dim):
        ar = np.abs(rfft_normradius[:,k])
        power_normradius = power_normradius + prefac * ar**2
        ap = np.abs(rfft_normpower[:,k])
        power_normpower = power_normpower + prefac * ap**2
    return(points_centered, points_normradius, points_normpower,
           power_normradius, power_normpower)
...
def write_boundary(pdf, coord_file, points_grid, points_irfft,
                   points_centered, points_normradius, points_normpower,
                   power_normradius, power_normpower):
    ...
    x = list(points_irfft[:,0])
    y = list(points_irfft[:,1])
    x.append(x[0])
    y.append(y[0])
    ...
    points = [ points_centered, points_normradius, points_normpower ]
    title = [ 'Centered', 'Norm Radius', 'Norm Power']
    subplot = [ 322, 323, 324 ]
    
    for (p, t, s) in zip(points, title, subplot):
    ...
    xx = range(2,11)    
    plt.subplot(325)
    plt.plot(xx, power_normradius[xx])
    plt.xlabel('k')
    plt.ylabel('Power')
    plt.title('Power, norm radius')
    plt.subplot(326)
    plt.plot(xx, power_normpower[xx])
    plt.xlabel('k')
    plt.ylabel('Power')
    plt.title('Power, norm power')
...
def recalculate(args):
    ...
    for (coord_file, image_file) in coord_image_pairs:
        ...
        (points_centered, points_normradius, points_normpower,
         power_normradius, power_normpower) = normalize_points(points_rfft, area)

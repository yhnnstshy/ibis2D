...
def plot_results(outdir, results):
    ...
    fields = ['circularity','power','pixelcnt','pixelmean', 'pixelstdev', 'pixelmedian', 'pixelmad' ]
    ...
    # histogram of pixelcnt, large vs. small
    zoom = dict()
    for s in samples:
        ctn = results[s]['ctnnum']
        z = 'large'
        if (ctn >= '037' and ctn <= '041') or (ctn >= '047'):
            z = 'small'
        zoom[s] = z
    
    for z in ('small', 'large'):
        f = 'pixelcnt'
        subset = [ s for s in samples if zoom[s] == z ]
        x = [ float(results[s][f]) for s in subset ]
        logger.info('pixelcnt %s median %f', z, np.median(x))
        plt.figure()
        plt.hist(x, bins=100)
        plt.title(' '.join([f, z]))
        plt.xlabel(f)
        plt.ylabel('Count of organoids')
        plt.savefig( os.path.join(outdir, 'FIGURES', '_'.join(['hist',f,z])))
        plt.close()        
...
def recalculate(args):
    ...
    for (coord_file, image_file) in coord_image_pairs:
        ...
        if image_file is not None:        
            ...
            plt.savefig( os.path.join(args.outdir, 'HISTOGRAMS', img_root + '_hist.pdf'))
            plt.close()
            minval = np.amin(img)
            maxval = np.amax(img)
            logger.info('min %f max %f', minval, maxval)

            boundary_pil = Image.new('1', (height, width), 0)
            inside_pil = Image.new('1', (height, width), 0)
            ImageDraw.Draw(boundary_pil).polygon(points_xy, outline=1, fill=None)
            ...
            plt.savefig( os.path.join(args.outdir, 'HISTOGRAMS', img_root + '_inside_hist.pdf'))
            plt.close()
            
            # add boundary to image
            img_boundary = np.copy(img)
            img_boundary[ boundary_np ] = maxval
            #for (r, c) in points_boundary:
            #    img[r, c] = maxval
            #img_pdf = os.path.join(args.outdir, 'IMAGES', img_root + '_boundary.pdf')
            #logger.info('writing to %s', img_pdf)
            #mpimg.imsave(img_pdf, img_boundary, cmap=mpcm.gray)
            
            # add fill to image
            img_fill = np.copy(img)
            img_fill[ inside_np > 0 ] = maxval
            img_fill_pdf = os.path.join(args.outdir, 'IMAGES', img_root + '_fill.pdf')
            #logger.info('writing %s', img_fill_pdf)
            #mpimg.imsave(img_fill_pdf, img, cmap=mpcm.gray)
            
            # write images as figures

...
def plot_results(outdir, results_dict):
    fields = dict()
    samples = sorted(results_dict.keys())
    abbrevs = [ results_dict[s]['orgnum'] for s in samples ]
    for s in samples:
        for f in results_dict[s].keys():
            if f == 'abbrev':
                continue
            fields[f] = fields.get(f, 0) + 1
    fields_list = sorted(fields.keys())
    logger.info('plot fields: %s', str(fields_list))
    fp = open(os.path.join(outdir, 'results_table.txt'), 'w')
    fp.write('\t'.join(['filename','abbrev'] + fields_list) + '\n')
    for s in samples:
        toks = [s] + [ str(results_dict[s][v]) for v in ['abbrev'] + fields_list ]
        fp.write('\t'.join(toks) + '\n')
    fp.close()
    ...
    for i in range(nfields):
        for j in range(nfields):
            ...
            f1 = fields_list[i]
            f2 = fields_list[j]
            x = [ results_dict[s].get(f1, 0) for s in samples ]
            y = [ results_dict[s].get(f2, 0) for s in samples ]
            plt.figure()
            plt.axis((min(x),max(x),min(y),max(y)))
            #plt.axes.set_xlim(min(x),max(x))
            #plt.axes.set_ylim(min(y),max(y))
            # plt.plot(x, y, 'ko')

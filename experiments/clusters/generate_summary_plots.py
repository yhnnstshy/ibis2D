...
def plot_ellipse(sp, x, y, ids, xlabel, ylabel):
...
def cluster_(array):
    ...
    print kmeans.fit_transform(array)
    return kmeans
...
def main():
    ...
    parser.add_argument('--indir', help='input directory', default='.', required=False)
    parser.add_argument('--outdir', help='output directory', default='.', required=False)
    ...
    for f in files:
        ...
        data = read_file(f)
        inv = np.array(data['Invasion']).astype(np.float)
        in_data = inv.reshape(inv.size, 1)
        labels = data['ID']
        kmeans = cluster_(in_data)
        return None
        ...
        for (sp, xname) in zip( sps, ('K14 Sum Peripheral Pixels', 'Fractional Area', 'K14 Sum Central Pixels', 'K14 Total Sum', 'K14 Mean') ):
            plot_ellipse(sp, data[xname], data['Invasion'], data['ID'], xname, 'Invasion')
        pdf.savefig()
        plt.close()
    pdf.close()             
